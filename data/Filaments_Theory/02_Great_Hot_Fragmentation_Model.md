...
كسر التناظر الحتمي: البنية الأولية للكون
...
النواة المركزية:
$$\rho{\text{core}}(r) = \rho0 \exp\left(-\frac{r^2}{R_c^2}\right)$$

الفضاء المحيط:
$$\rho{\text{void}}(r) = \rho{\infty} \left(1 - \exp\left(-\frac{r^2}{R_v^2}\right)\right)$$

حيث $\rho0 \gg \rho{\infty}$ و $Rc \ll Rv$.
...
الآلية الديناميكية: الصراع بين التماسك والشد

3.1 طبيعة النواة المركزية

النواة المركزية ليست بنية مستقرة وهادئة، بل هي مفاعل كوني فائق الحرارة:

$$\boxed{T{\text{core}} = \frac{G Mc mP}{kB R_c} \sim 10^{32} \text{ K}}$$
...
تطور معامل المقياس:
$$a(t) = a0 \left(\frac{t}{t0}\right)^{2/3}$$

في حالة الهيمنة المادية، متوافق مع الرصد.

البنية الهرمية والتجانس الناشئ
...
التعبير الرياضي:
$$\rho(r) = \rho{\text{local}} \left(1 - \frac{r}{R{\text{universe}}}\right)^{\alpha}$$

حيث $\alpha \approx 0.1$ و $R_{\text{universe}}$ هو نصف قطر الكون المرصود.

إعادة تفسير الأدلة الكونية
...
$$\boxed{I{\text{CMB}} = \sum{\text{fragments}} I_{\text{thermal}}(\text{fragment})}$$

آلية التكوين:
الشظايا النارية تشع حرارياً أثناء انتشارها
تخلق "حماماً حرارياً" متجانساً يملأ الكون
عند $T \approx 3000$ K، يصبح الكون شفافاً
ينطلق الإشعاع الذي نرصده اليوم
...
عدم التجانس على المقاييس الفائقة:
...
اختلافات في وفرة العناصر الخفيفة:
...
9.2 التحديات
التعقيد الحسابي: نمذجة التفتت المتعدد المقاييس
المعايرة: تحديد المعاملات الأولية للنواة
التوافق: ضمان التوافق مع جميع الرصدات الحالية
...
لقد نجحنا في:
حل مفارقة التفرد: تقديم آلية فيزيائية واضحة لنشأة الكون
تفسير التجانس: كخاصية ناشئة وليس مفترضة
إعادة تفسير الأدلة: CMB و BBN من منظور جديد
تقديم تنبؤات: قابلة للاختبار والتحقق التجريبي
...
الأولويات الفورية:
تطوير محاكاة حاسوبية شاملة لعملية التفتت
حساب التنبؤات الكمية الدقيقة
تصميم تجارب للتحقق من التنبؤات الأساسية
