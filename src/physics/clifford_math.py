...
class Multivector22:
    """
    Clifford Multivector for 22D SPEC Space (adapted for Mirnan).
    ...
    def __init__(self, scalar=0.0, vector=None, bivector=None, pseudoscalar=0.0):
        ...
        if vector is not None:
            v_arr = np.array(vector, dtype=float)
            if len(v_arr) > self.DIM:
                self.v = v_arr[:self.DIM]
            elif len(v_arr) < self.DIM:
                self.v = np.pad(v_arr, (0, self.DIM - len(v_arr)))
            else:
                self.v = v_arr
        else:
