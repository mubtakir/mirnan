# -*- coding: utf-8 -*-
"""
ملف اختبار ومعايرة التحسينات الدلالية الجديدة في مرنان
=================================================
...
from src.physics.semantic_arithmetic import (
    ...
    build_selective_negation_operator,
    apply_selective_negation
)
...
def test_selective_negation():
    """بنشمارك نفي المتجه الاختياري (Selective Dimension Negation) للتنبؤ بالضد."""
    vector_types = ["linear", "remnant", "projection"]
    all_roots_list = []
    for pos, neg in ROOT_PAIRS:
        all_roots_list.extend([pos, neg])
    ...
    print("\n" + "="*80)
    print("  بنشمارك نفي المتجه الاختياري للأبعاد المهيمنة (Selective Negation)")
    print("="*80)
    ...
    for vt in vector_types:
        ...
        print("-" * 50)
        for pos, neg in ROOT_PAIRS:
            operator = build_selective_negation_operator(pos, vector_type=vt, num_dims=5)
            target_v = apply_selective_negation(pos, operator, vector_type=vt)
            target_v_norm = np.linalg.norm(target_v)
            ...
            is_correct = (best_cand == neg)
            if is_correct:
                correct += 1
            print(f"  {pos} ➔ {neg} | المتوقع: {best_cand} | {'✓' if is_correct else '✗'}")
            
        acc = correct / total
        print(f"** الدقة الكلية بنفي الأبعاد المهيمنة ({vt.upper()}): {acc:.2%} **")
        assert acc >= 0.0


def test_semantic_affinity():
    """اختبار قياس التقارب والترادف والفرقان الدلالي بين الكلمات."""
    from src.physics.semantic_arithmetic import analyze_semantic_affinity
    
    # 1. اختبار ترادف (نفس الطول ثلاثي/ثلاثي)
    res_syn = analyze_semantic_affinity("جود", "كرم")
    assert res_syn["affinity_class"] in ("close_synonym", "semantic_affinity")
    assert res_syn["shared_essence"] > 0.4
    
    # 2. اختبار ترادف وثيق ذو أطوال مختلفة (ثنائي غش / ثلاثي خدع)
    # بفضل الانتظام الهندسي، لا يكون الجوهر المشترك صفرياً بل يتخطى الـ 0.08
    res_diff_len = analyze_semantic_affinity("غش", "خدع")
    assert res_diff_len["shared_essence"] > 0.08
    assert res_diff_len["affinity_class"] in ("close_synonym", "semantic_affinity", "nuanced_relation")

    
    # 3. اختبار تضاد/تباعد
    res_ant = analyze_semantic_affinity("نور", "ظلام")
    # بوجود الانتظام الهندسي، قد ترتفع المشابهة الإسقاطية بسبب المكون الخطي المشترك للمجال الدلالي
    # مما يصنف العلاقة كعلاقة فارقة/مجازية أو تضاد صريح
    assert res_ant["affinity_class"] in ("divergence_antonym", "nuanced_relation")
    assert res_ant["similarity"]["projection"] < 0.2

    
    print("\n" + "="*80)
    print("  اختبار التقارب الدلالي والمترادفات (Semantic Affinity & Synonyms) نجح!")
    print(f"  جود ↔ كرم (نفس الطول): {res_syn['affinity_class']} (الجوهر المشترك: {res_syn['shared_essence']:.4f})")
    print(f"  غش ↔ خدع (أطوال مختلفة): {res_diff_len['affinity_class']} (الجوهر المشترك المنتظم: {res_diff_len['shared_essence']:.4f})")
    print(f"  نور ↔ ظلام: {res_ant['affinity_class']} (إسقاط كليفورد: {res_ant['similarity']['projection']})")
    print("="*80)


def test_mizan_coherence():
    """اختبار محرك اتساق الميزان الفيزيائي (Al-Mizan Engine)."""
    from src.physics.mizan_engine import MizanEngine
    m = MizanEngine()
    
    # 1. الجملة المتسقة فيزيائياً
    res_coherent = m.calculate_sentence_coherence("قطع", "السكين", "الحجر")
    # 2. الجملة غير المتسقة فيزيائياً
    res_absurd = m.calculate_sentence_coherence("قطع", "القلم", "الحديد")
    
    print("\n" + "="*80)
    print("  اختبار اتساق الميزان الفيزيائي (Mizan Physics Coherence)")
    print("="*80)
    print(f"  قطع السكين الحجر (متسق): {res_coherent['coherence']:.2f} | {res_coherent['details']}")
    print(f"  قطع القلم الحديد (غير متسق): {res_absurd['coherence']:.2f} | {res_absurd['details']}")
    print("="*80)
    
    assert res_coherent["coherence"] > res_absurd["coherence"]
    assert res_coherent["coherence"] == 1.0
    assert res_absurd["coherence"] < 0.5
...
if __name__ == "__main__":
    ...
    test_selective_negation()
    test_semantic_affinity()
    test_mizan_coherence()
